...
@WebServlet(value = "/index6")

public class index6 extends HttpServlet {
    private String getgrade(int point) {
        if (point >= 90) return "A";
        else if ((point >= 75) && (point <= 89)) return "B";
        else if ((point >= 60) && (point <= 74)) return "C";
        else if ((point >= 50) && (point <= 59)) return "D";
        else return "F";
    }

    @Override
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        String name = request.getParameter("name_6");
        int point = Integer.parseInt(request.getParameter("age_6"));

        String outStr = name + " got " + getgrade(point) + " for exam";
